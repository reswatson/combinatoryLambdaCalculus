...
module comboLambda+globals where
...
-- A combinator:
isCombinator-like : Term → Bool
isCombinator-like Self = false
isCombinator-like (! n) = true 
isCombinator-like (` x) = false
isCombinator-like (Λ n trm) = true
isCombinator-like (trm1 · trm2) = isCombinator-like trm1 ∧ isCombinator-like trm2

isCombinator : Term → Bool
isCombinator Self = false
isCombinator (! n) = false 
isCombinator (` x) = false
isCombinator (Λ n trm) = true
isCombinator (trm1 · trm2) = isCombinator trm1 ∧ isCombinator-like trm2
...
CombinatorScope0 : {trm : Term} → isCombinator-like trm ≡ true → (Scope trm <= zero) ≡ true
...
Comp : {trm1 : Term} -> {trm2 : Term} → isCombinator-like trm1 ≡ true → isCombinator-like trm2 ≡ true -> Term
...
CompComb : {A B : Term} → (isc1 : isCombinator-like A ≡ true) → (isc2 : isCombinator-like B ≡ true) → isCombinator (Comp {A}{B} isc1 isc2) ≡ true
